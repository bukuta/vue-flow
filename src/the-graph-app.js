...
const ModalBG = require('./the-graph-modalbg').ModalBG;
...
const hotKeys = {
  ...
  // Delete
  46(app) {
    const graph = app.refs.graph.props.graph;
    const selectedNodes = app.refs.graph.state.selectedNodes;
    const selectedEdges = app.refs.graph.state.selectedEdges;
    const menus = app.props.menus;
...
module.exports.register = function (context) {
  ...
  TheGraph.App = React.createFactory(createReactClass({
    ...
    changeTooltip(event) {
      const tooltip = event.detail.tooltip;
